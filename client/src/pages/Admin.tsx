      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => !open && closeDialog()}>
        ...
        <DialogContent>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div><Label>Proficiência (0-100)</Label><Input type="number" min={0} max={100} value={form.proficiency} onChange={(e) => setForm((f) => ({ ...f, proficiency: Number(e.target.value) || 0 }))} /></div>
...
function AdminMessagesTab() {
  ...
  return (
    <div className="rounded-lg border overflow-hidden">
      <Table>
        <TableHeader>
          <TableRow><TableHead>Nome</TableHead><TableHead>Email</TableHead><TableHead>Mensagem</TableHead><TableHead>Data</TableHead></TableRow>
        </TableHeader>
        <TableBody>
          {messages?.map((m) => (
            <TableRow key={m.id}>
